...
Assigned per session and listed in the course schedule below. Each session's materials are placed in that week's `Required/` folder in the course repository, or linked directly. **These must be reviewed before class**, as they are essential to a meaningful in-class discussion, which constitutes a significant portion of your final grade.
...
- You will need to create a free [Claude account](https://claude.ai/) and [a Gemini API account](https://aistudio.google.com/) before your first class. Further instructions on proper API usage are given in class.
...
The project is assessed once, at the end of the term: teams present their completed work to the class across **Mon Nov 9 and Mon Nov 16**, address questions, and gather feedback, then incorporate the class and instructor feedback into the application and submit a final report. **The written report is due Monday, November 30**, two weeks after the last presentation session. There is no class meeting that day; the report is submitted through Blackboard.
...
A more detailed rubric will be provided. Each student is expected to fully grasp, discuss, defend, and take ownership of the entire project. All team members must therefore actively engage in various aspects of their projects, including research, coding, documentation, and presentation. Significant imbalances in the distribution of responsibilities or the type of work undertaken by team members will result in penalties.
...
- **The final project report** is capped at a **20%** deduction, beyond which a grade of **0** is assigned. A report that arrives more than two days late is not accepted.
- **Every other project deliverable** — team formation and the project proposal — is capped at a **30%** deduction and is **still expected**. The deduction stops growing at 30%; a deliverable that is never submitted at all receives the full deduction and forfeits its feedback. These deliverables carry no grade of their own, so the deduction is applied to your final project grade.
